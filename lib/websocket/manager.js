...
class WebSocketManager {
  ...
  addDashboardClient(ws) {
    ...
    console.log(
      `📊 Dashboard client connected (total: ${this.dashboardClients.size})`,
    );
  }
  ...
  removeDashboardClient(ws) {
    ...
    console.log(
      `📊 Dashboard client disconnected (total: ${this.dashboardClients.size})`,
    );
  }
    ...
    try {
      ...
      this.broadcastToDashboards({
        ...
        data: {
          ...
          signalStrength,
          networkType,
          sims: this.formatSims(sims), // ← added
          lastSeen: new Date(),
        },
      });
    } catch (error) {
    ...
    try {
      ...
      if (!deviceId || !sender || !content) {
        return this.send(ws, {
          type: "error",
          data: {
            code: "INVALID_SMS_DATA",
            message: "Missing required SMS data",
          },
        });
      }
      ...
      console.log(
        `✅ SMS saved: ${sender} -> ${receiverNumber} (Device: ${deviceId})`,
      );
  ...
  async handleCallForwardingResponse(ws, data) {
    try {
      ...
      console.log(`📞 Call forwarding response from ${deviceId}: action=${action}, success=${success}, simSlot=${simSlot}`);
      ...
      // Optionally update device record with call forwarding status
      // Note: Android simSlot is 0-based (0, 1), sims array is also 0-indexed
      const simIndex = simSlot; // No -1 needed, Android uses 0-based slots
      if (success && action === "forward" && phoneNumber) {
        await Device.findOneAndUpdate(
          { deviceId },
          {
            $set: {
              [`sims.${simIndex}.callForwardingTo`]: phoneNumber,
              [`sims.${simIndex}.callForwardingActive`]: true
            }
          }
        );
        ...
        await Device.findOneAndUpdate(
          ...
          {
            $set: {
              [`sims.${simIndex}.callForwardingTo`]: null,
              [`sims.${simIndex}.callForwardingActive`]: false
            }
          }
        );
      }
...
// Export getter for accessing instance from API routes
